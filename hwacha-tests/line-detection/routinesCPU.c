	...
	for(i=2; i<height-2; i++)
		for(j=2; j<width-2; j++)
		{
			...
			/*for(int i=0; i < DATA_SIZE_NR_row; i++){
				vec_mul_asm(DATA_SIZE_NR_col, mask_NR[i], data_NR[i]);
		...
		for(j=2; j<width-2; j++)
		{
			...
			*/
			...
			/*for(int i=0; i < DATA_SIZE_Gx_row; i++){
				vec_mul_asm(DATA_SIZE_Gx_col, mask_Gx[i], data_Gx[i]);
			}*/
			...
			/*for(int i=0; i < DATA_SIZE_Gy_row; i++){
				vec_mul_asm(DATA_SIZE_Gy_col, mask_Gy[i], data_Gy[i]);
			}*/
...
void draw_lines(uint8_t *imgtmp, int *x1, int *y1, int *x2, int *y2, int nlines)
{
	int x, y, wl, l;
	int width_line=9;
...
void line_asist_CPU(uint8_t *im, 
	uint8_t *imEdge, 
	float *sin_table, float *cos_table, 
